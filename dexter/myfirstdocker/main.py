...
def run_test_case(solution, input_path, output_path, return_value):
    try:
        ...
        if sys.argv[1] == 'python':
            ...
            run_command = ["java", "-classpath",
                           solution_vals[0], solution_vals[1]]
            time_limit = 2
        ...
        popen = Popen(run_command, stdin=PIPE, stdout=PIPE,
...
if __name__ == '__main__':
                ...
                for dirpath, _, files in os.walk(directory):
                    for file in files:
                        if file.endswith('.java'):
                            subprocess.run(
                                ["javac", os.path.join(dirpath, file)])
        ...
        for dirpath, _, files in os.walk(os.getcwd()):
            for file in files:
                if file.endswith('.cpp'):
                    res = subprocess.run(
                        ["clang++", "-o", "Solution", os.path.join(dirpath, file)], stderr=PIPE)
                    if len(res.stderr) > 5:
    ...
    while (True):
        ...
        folder_path = os.path.join(os.path.dirname(
            os.path.abspath(__file__)), 'tests', str(test_num))
